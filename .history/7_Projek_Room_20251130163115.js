...
modeload.load('./andy_room.gltf', item => {
    ...
    item.scene.traverse(i => {
        if (i.name === "Desk") {
            ...
            // posisi box relatif ke meja
            const deskBox = new THREE.Box3().setFromObject(i); // bounding box meja
            const deskHeight = deskBox.max.y - deskBox.min.y;
    
            // tempatkan box tepat di atas meja
            box.position.set(0, deskHeight / 2 + 0.5, 0); // 0.5 = setengah tinggi box
