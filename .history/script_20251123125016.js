...
// Updated gposi function (parameter order like boxes)
function gposi(cw, cs, ch, bh, s, i, j, k, n) {
    return {
        x: (-cw / 2 + i * cw + cw / 2) + s.position.x,
        z: (-ch / 2 + j * ch + ch / 2) + s.position.z,
        y: (bh / 2) + (k * bh),      // for stacked boxes
        y2: cs + s.position.y,        // for hover boxes
        newname: `${n}-${i}-${j}`
    };
}
...
function createHover(s, w, h, wg, hg, n) {
    ...
    for (let i = 0; i < wg; i++) {
        for (let j = 0; j < hg; j++) {
            ...
            const pos = gposi(cw, cs, ch, bh, s, i, j, 0, n);
    ...
    for (let i = 0; i < wg; i++) {
        for (let j = 0; j < hg; j++) {
            for (let k = 0; k < stacked; k++) {
                ...
                const pos = gposi(bw, bh, bd, bh, p, i, j, k, n);
