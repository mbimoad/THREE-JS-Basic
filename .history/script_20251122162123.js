...
loader.load(modelrack.href, item => {
    ...
    let pan = wadah.length - 1; 
    let gap = tinggi / pan; 
    for(let i=0; i<=pan; i++) wadah[i].position.y = i * gap;
    createBox(0.009, true, wadah[0], 0.5, 0.5, 0);
    createBox(0.5, true, wadah[1], 0.5, 0.5, 0);
})

const createBox = (w,m,p,x,y,z) => {
    y = y+w/2; 
    if(m) {
        const boxs = boxmesh(w,w,w, true); 
        boxs.position.set(x,y,z); 
        p.add(box)
    } else {
