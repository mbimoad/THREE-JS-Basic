...
modeload.load('./rack1/scene.gltf', item => {
    ...
    datasbox.forEach(data => {
        ...
        if(obj != null) {
            ...
            obj.getWorldPosition(centerWorld);
...
window.addEventListener('dblclick', function(e) {
    ...
    const s = boxes(w, b, h, Math.random() * 0xFFFFFF, false, 1);
    console.log(w,h,b);
    // pastikan geometry punya bounding box
    s.geometry.computeBoundingBox(); 

    // buat Box3 dari geometry
    const bbox = s.geometry.boundingBox;

    // ambil ukuran
    const size = new THREE.Vector3();
    bbox.getSize(size);

    console.log("Width, Height, Depth:", size.x, size.y, size.z);
