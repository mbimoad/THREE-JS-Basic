...
window.addEventListener('dblclick', function(e) {
    ...
    const hov   = intersects[0].object; 
    const shelf = hov.userData.ww;      // ← parent asli (Shelf_Supports)
    ...
    const w = hov.userData.cw * 0.4;
    const h = hov.userData.ch * 0.4;
    ...
    hov.getWorldPosition(centerWorld);

    // --- konversi world → local parent (shelf) ---
    const centerLocal = shelf.worldToLocal(centerWorld.clone());

    // --- tempatkan box tepat di tengah cell + stack ke atas ---
    s.position.set(
        centerLocal.x,
