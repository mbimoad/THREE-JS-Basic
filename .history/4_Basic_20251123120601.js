...
loader.load('./rack1/scene.gltf', item => {
    ...
    item.scene.traverse(i => {
        if (i.name.includes("Shelf_Supports")) {
            ...
            const planes = createPlaneGeometryInside(i, parent, 3, 2);
            createHover(planes, parent.x, 1, 3, 2, i.name);
        }
    });
});


function gposi(w, h, cw, ch, bh, p, i, j, k, n) {
    return {
...
function createHover(p, w, h, wg, hg, n) {
    const cw = w / wg;
    const ch = h / hg;
    const cs = 0.01;
    const bh = 0.3;
    for (let i = 0; i < wg; i++) {
        for (let j = 0; j < hg; j++) {
            ...
            const pos = gposi(w, h, cw, ch, bh, p, i, j, 0, n);
            hov.position.set(pos.x, cs + p.position.y, pos.z);
...
function createPlaneGeometryInside(parent, col, row) {
