...
function createHover(s,w,h,wg,hg,n,boxH) {
...
modeload.load('./rack1/scene.gltf', item => {
    ...
    let boxH = gaps/stackeds // gap dibagi jumlah maximal box
    ...
    item.scene.traverse(i => {
        if(i.name.includes("Shelf_Supports")) {
            ...
            createHover(s, p.x, 1, 3, 2, i.name, boxH);
        }
    })
})
...
window.addEventListener('dblclick', function(e) {
    ...
    if(hoverbox.includes(intersects[0].object)) {
        const count = storebox[intersects[0].object.name]; 
        if(count >= stackeds) alert("Max"); 
        else {
            const w = intersects[0].object.userData.cw * 0.8;
            const h = intersects[0].object.userData.ch * 0.8;
            const b = intersects[0].object.userData.bh;
            const s = boxes(w,b,h, Math.random() * 0xFFFFFF, false, 1);
            s.position.set(
                intersects[0].object.position.x,
                intersects[0].object.position.y + (b/2) + (count * b),
