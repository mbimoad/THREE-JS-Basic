...
function gposi(w, h, cw, ch, boxHeight, ground, i, j, k, key) {
    return {
        x: (-w / 2 + i * cw + cw / 2) + ground.position.x,
        z: (-h / 2 + j * ch + ch / 2) + ground.position.z,
...
function createHoverToPlaneGeometry(ground, w, h, Gwsegmn, Ghsegmn, key) {
    ...
    for (let i = 0; i < Gwsegmn; i++) {
        for (let j = 0; j < Ghsegmn; j++) {
            ...
            const pos   = gposi(w, h, cw, ch, boxHeight, ground, i, j, 0, key);
    ...
    for (let i = 0; i < Gwsegmn; i++) {
        for (let j = 0; j < Ghsegmn; j++) {
            for (let k = 0; k < stacked; k++) {
                ...
                const pos = gposi(w, h, cw, ch, boxHeight, ground, i, j, k, key);
