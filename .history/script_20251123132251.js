...
// Fungsi gposi sudah disesuaikan urutannya: width, height, depth pertama
function gposi(bw, bh, bd, w, h, boxHeight, s, i, j, k, n, cs=0) {
    const y = boxHeight / 2 + k * boxHeight;
    return {
         x: (-w / 2 + i * bw + bw / 2) + s.position.x,
         z: (-h / 2 + j * bd + bd / 2) + s.position.z,
         y: y,
        y2: y  + s.position.y,
        y3: cs + s.position.y,
        newname: `${n}-${i}-${j}`
