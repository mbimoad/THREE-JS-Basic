...
// Contoh penggunaan
const mesh = s; // mesh yang sudah dibuat
...
modeload.load('./rack1/scene.gltf', item => {
    ...
    datasbox.forEach(data => {
        const obj = findRackByName(data.rack);
        if(obj != null) {
            const count = storebox[obj.name];     
            const w   = cw * 0.5; 
            const h   = ch * 0.5; 
            const b   = bh; 
            ...
            const centerLocal = obj.worldToLocal(c.clone());
            s.position.set(
                centerLocal.x,
