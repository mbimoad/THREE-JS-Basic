...
const boxWidths  = Math.min(0.5, cellwidths * 0.8); 
const boxDepths  = Math.min(0.5, cellheight * 0.8); 
const boxHeight  = 0.3; 
...
const GetPositionXYZ = (ground, i, j, k, nem) => {
    return {
        x1: (-Gwidths/2 + i * cellwidths + cellwidths / 2) + ground.position.x, 
        z2: (-Gheight/2 + j * cellheight + cellheight / 2) + ground.position.z, 
        y2: boxHeight/2 + k * boxHeight, 
        name: `${nem}-${i}-${j}`
    }
}
    ...
    for(let i=0; i<Gwsegmn; i++) {
        for(let j=0; j<Ghsegmn; j++) {
            for(let k=0; k<stacked; k++) {
                ...
                const {x,y,z, newname} = GetPositionXYZ(Gwidths, Gheight, cellwidths, cellheight, boxHeight, ground, i, j, k, name); 
                box.position.set(
                    x + ground.position.x,
                    y + ground.position.y, 
                    z + ground.position.z
