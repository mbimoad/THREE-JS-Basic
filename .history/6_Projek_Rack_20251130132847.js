...
const datasbox = [
    {
        "rack_name": "Shelf_Supports_0_3-2-1",
        "position": new THREE.Vector3(0.265150189399719, 0.18979999930858618, 0.35971172650655125)
    },
    {
        "rack_name": "Shelf_Supports_0_3-1-1",
        "position": new THREE.Vector3(0.2651501893997188, 0.18979999930858618, 0)
    },
    {
        "rack_name": "Shelf_Supports_0_3-0-1",
        "position": new THREE.Vector3(0.2651501893997188, 0.18979999930858618, -0.35971172650655114)
    },
    {
        "rack_name": "Shelf_Supports_1_2-0-1",
        "position": new THREE.Vector3(0.26515018939971924, 0.18979999930858624, -0.35971172650655103)
    },
    {
        "rack_name": "Shelf_Supports_1_2-1-1",
        "position": new THREE.Vector3(0.26515018939971946, 0.18979999930858624, 0)
    },
    {
        "rack_name": "Shelf_Supports_1_2-2-1",
        "position": new THREE.Vector3(0.26515018939971946, 0.18979999930858624, 0.35971172650655103)
    },
    {
        "rack_name": "Shelf_Supports_1_2-2-1",
        "position": new THREE.Vector3(0.26515018939971946, 0.3493999979257585, 0.35971172650655103)
    },
    {
        "rack_name": "Shelf_Supports_1_2-1-1",
        "position": new THREE.Vector3(0.26515018939971946, 0.3493999979257585, 0)
    },
    {
        "rack_name": "Shelf_Supports_1_2-0-1",
        "position": new THREE.Vector3(0.26515018939971924, 0.3493999979257585, -0.35971172650655103)
    },
    {
        "rack_name": "Shelf_Supports_2_3-0-1",
        "position": new THREE.Vector3(0.2651501893997188, 0.18979999930858601, -0.35971172650655114)
    },
    {
        "rack_name": "Shelf_Supports_2_3-0-1",
        "position": new THREE.Vector3(0.2651501893997188, 0.34939999792575827, -0.35971172650655114)
    },
    {
        "rack_name": "Shelf_Supports_2_3-0-1",
        "position": new THREE.Vector3(0.2651501893997188, 0.5089999965429306, -0.35971172650655114)
    },
    {
        "rack_name": "Shelf_Supports_5_2-1-0",
        "position": new THREE.Vector3(0.7651501893997195, 0.18979999930858557, -2.220446049250313e-16)
    },
    {
        "rack_name": "Shelf_Supports_5_2-1-0",
        "position": new THREE.Vector3(0.7651501893997195, 0.3493999979257578, -2.220446049250313e-16)
    },
    {
        "rack_name": "Shelf_Supports_5_2-1-0",
        "position": new THREE.Vector3(0.7651501893997195, 0.5089999965429302, -2.220446049250313e-16)
    },
    {
        "rack_name": "Shelf_Supports_5_1-1-1",
        "position": new THREE.Vector3(0.2651501893997191, 0.18979999930858557, 0)
    },
    {
        "rack_name": "Shelf_Supports_5_0-2-1",
        "position": new THREE.Vector3(0.26515018939971946, 0.18979999930858557, 0.35971172650655103)
    }
];
...
function debugMeshSize(mesh) {
    // Buat bounding box dari mesh, termasuk scale dan transformasi
    const bbox = new THREE.Box3().setFromObject(mesh);
    const size = new THREE.Vector3();
    bbox.getSize(size);
    return size; // size.x = width, size.y = height, size.z = depth
}
...
modeload.load('./rack1/scene.gltf', item => {
    ...
    for (let i=0; i<=long; i++) {
        // Update namanya juga 
        wadah[i].name = "Shelf_Supports_" + i; 
        wadah[i].position.y = marginBawah + gaps * i;
    }
    ...
    datasbox.forEach(data => {
        const hov = hoverbox.find(h => h.name === data.rack_name); // pakai hover cell
        if(!hov) return;
    
        const count = storebox[hov.name];     
        const w     = hov.userData.cw * 0.5; 
        const h     = hov.userData.ch * 0.5; 
        const b     = hov.userData.bh; 
        const s     = boxes(w, b, h, Math.random() * 0xFFFFFF, false, 1);
    
        const c = new THREE.Vector3();
        hov.getWorldPosition(c);
        const centerLocal = hov.userData.ww.worldToLocal(c.clone()); // rak asli
        s.position.set(
            centerLocal.x,
            centerLocal.y + (b / 2) + (count * b),
            centerLocal.z
        );
        hov.userData.ww.add(s);
        storebox[hov.name]++;
    });
...
window.addEventListener('dblclick', function(e) {
    ...
    const c = new THREE.Vector3();
    intersects[0].object.getWorldPosition(c);
    const centerLocal = intersects[0].object.userData.ww.worldToLocal(c.clone());
