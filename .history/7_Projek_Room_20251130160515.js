...
modeload.load('./andy_room.gltf', item => {
    ...
    item.scene.traverse(obj => {
        if(obj.name === "Desk") {
            ...
            // Hitung bounding box meja
            const box3 = new THREE.Box3().setFromObject(obj);
            const size = new THREE.Vector3();
            const center = new THREE.Vector3();
            box3.getSize(size);     // ukuran meja
            box3.getCenter(center); // posisi tengah meja di world
            ...
            // Tempatkan box di atas meja
            // Posisi box relatif ke meja, jadi kita konversi world -> local
