...
window.addEventListener('keyup', e=> updateKeyboard())

function updateKeyboard(speed) {
    if(keyboard['w']) control.moveForward(speed); 
    if(keyboard['s']) control.moveForward(-speed);
    if(keyboard['a']) control.moveRight(-speed);
    if(keyboard['d']) control.moveRight(speed);
}
