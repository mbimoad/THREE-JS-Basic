...
function planeShadowed(parentObj, size, col, row) {
    ...
    // Posisi Shelf_Support di world
    const wp = new THREE.Vector3();
    parentObj.getWorldPosition(wp);
    mesh.position.y = wp.y += 0.5;

    // Offset final
    const offset = new THREE.Vector3(0.5, 0.1, 0);
    offset.applyQuaternion(parentObj.getWorldQuaternion(new THREE.Quaternion()));
    // mesh.position.add(offset);

    // --- tempel ke parent rack agar ikut clone/posisi ---
