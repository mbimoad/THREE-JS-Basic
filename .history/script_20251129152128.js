...
// Pointing 
const textureLoad = .load('./img/floor.jpg');
...
function createWalls(size, h = 5, t = 0.5, mat) {
    mat ||= new THREE.MeshStandardMaterial({ color: 0x888 });
    const g1 = new THREE.BoxGeometry(size, h, t);
    const g2 = new THREE.BoxGeometry(t, h, size);
    const y  = h / 2;

    const pos = [
        [0, y,  size/2, g1], // depan
        [0, y, -size/2, g1], // belakang
        [-size/2, y, 0, g2], // kiri
        [ size/2, y, 0, g2], // kanan
    ];

    const group = new THREE.Group();
    pos.forEach(([x,y,z,g]) => group.add(new THREE.Mesh(g, mat)).position.set(x,y,z));
