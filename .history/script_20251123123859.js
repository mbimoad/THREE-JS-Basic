...
/* ========================================================
   gposi() — urutan selalu (width, height, depth)
   ======================================================== */

function gposi(width, height, depth, w, h, s, i, j, k, n) {

    // X & Z tetap
    const x = (-w / 2 + i * width + width / 2) + s.position.x;
    const z = (-h / 2 + j * depth + depth / 2) + s.position.z;

    // Y terpisah
    const y1 = 0;                                 // dasar cell
    const y2 = (height / 2) + (k * height);       // dasar + stack
    const y3 = y2 + s.position.y;                 // final (dipakai objek)

    return { x, y1, y2, y3, z, newname: `${n}-${i}-${j}-${k}` };
}
...
function createHover(s, w, h, wg, hg, n) {
    const cw = w / wg;
    const ch = h / hg;
    const cs = 0.01;     // height hover
    const bh = 0.3;      // height box
    ...
    for (let i = 0; i < wg; i++) {
        for (let j = 0; j < hg; j++) {
            ...
            // gposi seperti boxes(cw, cs, ch)
            const pos = gposi(cw, cs, ch, w, h, s, i, j, 0, n);
    ...
    for (let i = 0; i < wg; i++) {
        for (let j = 0; j < hg; j++) {
            for (let k = 0; k < stacked; k++) {
                ...
                const pos = gposi(bw, bh, bd, w, h, p, i, j, k, n);
...
window.addEventListener("dblclick", e => {
    ...
    if (hoverBox.includes(obj)) {
        ...
        const pos = gposi(w, b, h, obj.userData.cw, obj.userData.ch, obj, 0, 0, count, obj.name);

        newBox.position.set(pos.x, pos.y3, pos.z);
