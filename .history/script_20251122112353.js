...
const gm = createGround(gs, 0xBBBBBB, 'ground', true, true)
...
window.addEventListener('mousemove', function(e){
    ...
    intersects.forEach(item => {
        if(item.object.name == 'ground') {
            ...
            gh.position.set(hp.x,0,hp.position.z); 
            if(check()) gh.material.color.set(0xFF0000) 
...
const animate = (time) => {
    ...
    objects.forEach(i => {
        if(i.type == "Mesh") {
            i.rotation.x = time/1000;
            i.rotation.y = time/1000;
        }
    })
