...
modeload.load('./andy_room.gltf', item => {
    ...
    item.scene.traverse(i => {
        if(i.name == "Desk") {
            const axes = new THREE.AxesHelper(10, 10); // ukuran 1 unit
            i.add(axes);

               // origin lokal
        console.log('Desk local origin:', i.position); 

        // bounding box global
        const bbox = new THREE.Box3().setFromObject(i);
        console.log('Desk bounding box min:', bbox.min, 'max:', bbox.max);

        // center bounding box
        const center = bbox.getCenter(new THREE.Vector3());
        console.log('Desk center in world coords:', center);
