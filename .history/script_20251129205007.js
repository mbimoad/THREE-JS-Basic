...
loader.load(modelrack.href, item => {
    ...
    let stackeds = 5; // max stack
    let marginBawah  = 0.4;
    let marginAtas   = 0.4;
    const pan = wadah.length - 1;
    let usableHeight = tinggi - marginAtas - marginBawah;
    let gap = usableHeight / pan;
    let boxH = (gap/stackeds) - 0.05;
    for(let i=0; i<=pan; i++) wadah[i].position.y = marginBawah + gap * i;
    createBox(0.009, 0.009 - boxH, true, wadah[1], 0.5,0,0)
    createBox(0.5, boxH, false, wadah[2], 0.5,0,0)
})
...
const createBox = (w,h,m,p,x,y,z) => {
    y = y+w/2; 
    if(!m) {
        const box = boxmesh(w,h,w,true); 
        box.position.set(x,y,z); 
        p.add(box)
    } else {
        loader.load(modelboxs.href, item => {
            item.scene.scale.set(w,h,w); 
