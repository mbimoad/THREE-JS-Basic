...
function createBoardHTML(side, title, results = []) {
    const htmlString = `
    ...
    // Gunakan DOMParser untuk parse string jadi elemen DOM
    const parser = new DOMParser();
    const doc = parser.parseFromString(htmlString, 'text/html');
    const boardElement = doc.body.firstChild;
    ...
    // Posisi dan rotasi
    if (side === 'kiri') {
        board.position.set(kiri.position.x + wallThickness/2 + 0.01, wallHeight/2, 0);
        board.rotation.y = Math.PI / 2;
    } else if (side === 'kanan') {
        board.position.set(kanan.position.x - wallThickness/2 - 0.01, wallHeight/2, 0);
        board.rotation.y = -Math.PI / 2;
    }
...
// Contoh pemakaian
createBoardHTML('kiri', 'Judul', ['1+1', '2']);
createBoardHTML('kanan', 'Judul', ['3+2', '5']);
