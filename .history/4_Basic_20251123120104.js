...
function gposi(w, h, cw, vh, boxHeight, ground, i, j, k, key) {
    return {
        x: (-w / 2 + i * cw + cw / 2) + ground.position.x,
        z: (-h / 2 + j * vh + vh / 2) + ground.position.z,
...
function createHoverToPlaneGeometry(ground, w, h, Gwsegmn, Ghsegmn, key) {
    ...
    for (let i = 0; i < Gwsegmn; i++) {
        for (let j = 0; j < Ghsegmn; j++) {
            const hover = boxes(cw, cellHovers, vh, 0x00FF00, true, 0.0); 
            const pos   = gposi(w, h, cw, vh, boxHeight, ground, i, j, 0, key);
            hover.position.set(pos.x, cellHovers + ground.position.y, pos.z);
            hover.name = pos.newname;
            hover.userData.cw = cw;
            hover.userData.vh = vh;
