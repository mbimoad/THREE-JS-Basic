...
const boxmesh = (w1,w2,w3,c) => new THREE.Mesh(new THREE.BoxGeometry(w,w,w), new THREE.MeshBasicMaterial({color: Math.random() * 0xFFFFFF, colorWrite: c}));

// Invisible walls
const createInvisibleWall = (x) => {
    const wall = boxmesh(8)
    new THREE.Mesh(
...
window.addEventListener('dblclick', function(e) {
    if(!check()){
        intersects.forEach(item => {
            if(item.object.name == 'ground') {
                let bc = box.clone(); 
                bc.position.copy(groundHov.position); 
                bc.position.y = 0.5;
...
for(let i=0; i<1; i++) {
    ...
    for(let j=0; j<2; j++) {
        ...
        for(let k=0; k<3; k++) {
            ...
            cell.position.set(gap2*j, 1, gap2*k); 
...
loader.load(modelurl.href, item => {
    ...
    item.scene.position.x = 10;
    item.scene.position.y = 5;
    ...
    const clonelvls = wadah[wadah.length - 1].clone(); 
    item.scene.add(clonelvls); 
    wadah.push(clonelvls); 

    const pan = wadah.length; 
    const gap = tinggi / (pan - 1); 
    for(let i=0; i<pan; i++) wadah[i].position.y = gap * i;
    
    wadah.forEach(item => addFullBox(item))
    createBox(0.009, true, wadah[1], 0.5,0,0)
    createBox(0.5, false, wadah[2], 0.5,0,0)
})
...
const createBox = (w, model, parent, posX, posY, posZ) => { 
    if (!model) {
        ...
        box.position.set(posX, posY + w / 2, posZ); 
        parent.add(box);
    } else {
        loader.load(modelbox.href, item => {
            item.scene.scale.set(w, w, w); 
            item.scene.position.set(posX, posY + w / 2, posZ); 
            parent.add(item.scene);
        });
    }
}
