...
modeload.load('./andy_room.gltf', item => {
    ...
    item.scene.traverse(i => {
        if (i.name === "Desk") {
            ...
            const deskHeight = bbox.max.y - bbox.min.y;
            ...
            // posisikan box di atas meja
            box.position.set(center.x, bbox.max.y + 0.5, center.z);
    
            // tambahkan ke scene langsung agar posisi world tepat
            item.scene.add(box);
        }
    });
