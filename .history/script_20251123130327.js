...
function gposi(cw, cs, ch, bh, w, h, s, i, j, k, n) {
    return {
        x: (-w / 2 + i * cw + cw / 2) + s.position.x,
        z: (-h / 2 + j * ch + ch / 2) + s.position.z,
        y: (bh / 2) + (k * bh),
        y2: cs + s.position.y, 
        y3: (bh / 2) + (k * bh) + s.position.y,
        newname: `${n}-${i}-${j}`
    };
}
function createHover(s, w, h, wg, hg, n) {
    ...
    for (let i = 0; i < wg; i++) {
        for (let j = 0; j < hg; j++) {
            ...
            const pos = gposi(cw, cs, ch, bh, w, h, s, i, j, 0, n);
            

            hov.position.set(pos.x, cs + s.position.y, pos.z);
...
function loopingBoxToPlaneGeometry(stacked, s, w, h, wg, hg, n) {
    ...
    const bd = Math.min(0.3, ch * 0.import * as THREE from 'three';
        import { OrbitControls } from 'three/examples/jsm/controls/OrbitControls';
        import { GLTFLoader } from 'three/examples/jsm/loaders/GLTFLoader';
        ...
        function gposi(w, h, cw, ch, bh, s, i, j, k, n) {
            return {
                x: (-w / 2 + i * cw + cw / 2) + s.position.x,
                z: (-h / 2 + j * ch + ch / 2) + s.position.z,
                y: (bh / 2) + (k * bh),
        ...
        function createHover(s, w, h, wg, hg, n) {
            ...
            for (let i = 0; i < wg; i++) {
                for (let j = 0; j < hg; j++) {
                    const hov = boxes(cw, cs, ch, 0x00FF00, true, 0.0); 
                    const pos = gposi(w, h, cw, ch, bh, s, i, j, 0, n);
                    hov.position.set(pos.x, cs + s.position.y, pos.z);
                    hov.name = pos.newname;
                    hov.userData.cw = cw;
                    hov.userData.ch = ch;
            ...
            for (let i = 0; i < wg; i++) {
                for (let j = 0; j < hg; j++) {
                    for (let k = 0; k < stacked; k++) {
                        const b   = boxes(cw, bh, bd, Math.random() * 0xFFFFFF, false, 1); 
    ...
    for (let i = 0; i < wg; i++) {
        for (let j = 0; j < hg; j++) {
            for (let k = 0; k < stacked; k++) {
                ...
                const pos = gposi(bw, bh, bd, bh, w, h, s, i, j, k, n);
                function gposi(w, h, cw, ch, bh, s, i, j, k, n) {
                    
                hov.position.set(pos.x, pos.y + s.position.y, pos.z);
                scenes.add(hov);
