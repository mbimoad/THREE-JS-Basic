...
function gposi(w, h, , cellheight, boxHeight, ground, i, j, k, key) {
    return {
        x: (-w / 2 + i *  +  / 2) + ground.position.x,
...
function createHoverToPlaneGeometry(ground, w, h, Gwsegmn, Ghsegmn, key) {
    ...
    for (let i = 0; i < Gwsegmn; i++) {
        for (let j = 0; j < Ghsegmn; j++) {
            const hover = boxes(, cellHovers, cellheight, 0x00FF00, true, 0.0); 
            const pos   = gposi(w, h, , cellheight, boxHeight, ground, i, j, 0, key);
...
function loopingBoxToPlaneGeometry(stacked, ground, w, h, Gwsegmn, Ghsegmn, key) {
    const  = w / Gwsegmn;
    const cellheight = h / Ghsegmn;
    const boxWidth = Math.min(0.3,  * 0.8);
    ...
    for (let i = 0; i < Gwsegmn; i++) {
        for (let j = 0; j < Ghsegmn; j++) {
            for (let k = 0; k < stacked; k++) {
                ...
                const pos = gposi(w, h, , cellheight, boxHeight, ground, i, j, k, key);
