...
function gposi(w, h, cw, cellheight, boxHeight, ground, i, j, k, key) {
    return {
...
function createHoverToPlaneGeometry(ground, w, h, Gwsegmn, Ghsegmn, key) {
    ...
    for (let i = 0; i < Gwsegmn; i++) {
        for (let j = 0; j < Ghsegmn; j++) {
            const hover = boxes(cw, cellHovers, cellheight, 0x00FF00, true, 0.0); 
            const pos   = gposi(w, h, cw, cellheight, boxHeight, ground, i, j, 0, key);
    ...
    for (let i = 0; i < Gwsegmn; i++) {
        for (let j = 0; j < Ghsegmn; j++) {
            for (let k = 0; k < stacked; k++) {
                ...
                const pos = gposi(w, h, cw, cellheight, boxHeight, ground, i, j, k, key);
