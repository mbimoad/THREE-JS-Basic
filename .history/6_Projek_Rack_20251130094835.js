...
const boxes = (w1,w2,w3,c,t,o) => new THREE.Mesh(
    new THREE.BoxGeometry(w1,w2,w3),
...
/* =======================================================================
    UPDATED: planeShadowed() sekarang menerima parentMesh (Shelf level)
======================================================================= */
function planeShadowed(parentSize, col, row, parentMesh) {   // ⬅ UPDATED
    const mesh = new THREE.Mesh(
        new THREE.PlaneGeometry(parentSize.x, 1, col, row), 
        new THREE.MeshBasicMaterial({color: 0xFFFFFF, wireframe: true})
    ); 
    mesh.rotation.x = -Math.PI/2; 
    mesh.position.copy(parentSize.cntr);
    parentMesh.add(mesh);   // ⬅ UPDATED (dulu: scenes.add)
    return mesh; 
}
...
/* =======================================================================
    UPDATED: createHover() sekarang parent = planeShadowed()
======================================================================= */
function createHover(parent, w, h, wg, hg, n, boxH, obj) {  // ⬅ UPDATED
    ...
    for(let i=0;i<wg;i++) {
        for(let j=0;j<hg;j++) {
            ...
            const pos = gposi(w,h,cw,ch,bh,parent,i,j,0,n, cs);  
            ...
            parent.add(hov);  // ⬅ UPDATED (dulu: scenes.add)
        }
    }
}
...
modeload.load('./rack1/scene.gltf', item => {
    ...
    let boxH = (gaps/stackeds) - 0.05;

    for (let i=0; i<=long; i++) 
        wadah[i].position.y = marginBawah + gaps * i;

    /* ============================================================
        MULTIPLE RACK GRID SYSTEM
    ============================================================ */
    const rackqtys = 4; 
    const gridSize = 2;
    const spacing  = 5;
    ...
    /* ============================================================
        APPLY planeShadowed + createHover based on rack level
    ============================================================ */
    racks.forEach((rack, index) => {
        ...
        rack.traverse(i => {
            if(i.name.includes("Shelf_Supports")) {
                ...
                // ⬅ UPDATED: plane menggunakan parent shelf_supports
                const s = planeShadowed(p, 3, 2, i);

                const n = i.name + "_" + index;

                // ⬅ UPDATED: hover menempel ke plane, bukan scene
