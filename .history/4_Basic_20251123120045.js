...
function gposi(d, Gheight, cellwidths, cellheight, boxHeight, ground, i, j, k, key) {
    return {
        x: (-d / 2 + i * cellwidths + cellwidths / 2) + ground.position.x,
        z: (-Gheight / 2 + j * cellheight + cellheight / 2) + ground.position.z,
        y: (boxHeight / 2) + (k * boxHeight),
        newname: `${key}-${i}-${j}`
    };
}
function createHoverToPlaneGeometry(ground, d, Gheight, Gwsegmn, Ghsegmn, key) {
    const cellwidths = d / Gwsegmn;
    ...
    for (let i = 0; i < Gwsegmn; i++) {
        for (let j = 0; j < Ghsegmn; j++) {
            ...
            const pos   = gposi(d, Gheight, cellwidths, cellheight, boxHeight, ground, i, j, 0, key);
...
function loopingBoxToPlaneGeometry(stacked, ground, d, Gheight, Gwsegmn, Ghsegmn, key) {
    const cellwidths = d / Gwsegmn;
    const cellheight = Gheight / Ghsegmn;
    ...
    for (let i = 0; i < Gwsegmn; i++) {
        for (let j = 0; j < Ghsegmn; j++) {
            for (let k = 0; k < stacked; k++) {
                ...
                const pos = gposi(d, Gheight, cellwidths, cellheight, boxHeight, ground, i, j, k, key);
...
function createGround(d, Gheight, Gwsegmn, Ghsegmn) {
    const g = new THREE.Mesh(
        new THREE.PlaneGeometry(d, Gheight, Gwsegmn, Ghsegmn),
