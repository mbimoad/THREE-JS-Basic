...
// gposi revisi: urutan parameter sama dengan boxes, y dihitung di sini
function gposi(cw, cs, ch, s, i, j, k, n) {
    const bh = cs; // tinggi kotak
    return {
        x: (-cw / 2 + i * cw + cw / 2) + s.position.x,
        z: (-ch / 2 + j * ch + ch / 2) + s.position.z,
        y2: cs + s.position.y + (k * bh),
        newname: `${n}-${i}-${j}`
    };
}
...
function createHover(s, w, h, wg, hg, n) {
    const cw = w / wg;
    const ch = h / hg;
    const cs = 0.01; // tinggi hover box
    const bh = 0.3;
    for (let i = 0; i < wg; i++) {
        for (let j = 0; j < hg; j++) {
            const hov = boxes(cw, cs, ch, 0x00FF00, true, 0.0); 
            const pos = gposi(cw, cs, ch, s, i, j, 0, n); // urutan parameter sudah sama
            hov.position.set(pos.x, pos.y2, pos.z); // pakai pos.y2
    ...
    for (let i = 0; i < wg; i++) {
        for (let j = 0; j < hg; j++) {
            for (let k = 0; k < stacked; k++) {
                ...
                const pos = gposi(bw, bh, bd, p, i, j, k, n); // urutan parameter sudah sama
                b.position.set(pos.x, pos.y2, pos.z); // pakai pos.y2
                stackedBoxes.push(b);
                scenes.add(b);
            }
        }
...
window.addEventListener("dblclick", e => {
    ...
    if (hoverBox.includes(obj)) {
        ...
        const pos = gposi(w, b, h, obj, 0, 0, count, obj.name); // hitung posisi y di gposi
        newBox.position.set(pos.x, pos.y2, pos.z);
