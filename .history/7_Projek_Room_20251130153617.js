...
camera.lookAt(orbit.target);`;
    ...
    if (n.clipboard && navigator.clipboard.writeText) {
      navigator.clipboard.writeText(code).then(() => alert("📋 Camera Di Copy", code))
    } else {
      alert.log("📋 Clipboard API tidak tersedia. Berikut kodenya:\n", code);
    }
}
