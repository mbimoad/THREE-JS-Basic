...
// Tambah lukisan
const multi_mater = [
    new THREE.MeshBasicMaterial({map: textureLoad.load('./img/goku.jpg')}),
    new THREE.MeshBasicMaterial({map: textureLoad.load('./img/goku.jpg')}),
    new THREE.MeshBasicMaterial({map: textureLoad.load('./img/goku.jpg')}),
    new THREE.MeshBasicMaterial({map: textureLoad.load('./img/goku.jpg')}),
    new THREE.MeshBasicMaterial({map: textureLoad.load('./img/goku.jpg')}),
...
const lukisan  = createCustomBox(1,1,1); 
const televisi = createCustomBox(5,3,1); 
lukisan.material = multi_mater; 
const video    = document.querySelector('video'); 
video.src = "bardoc.webm"; 
video.loop = true; 
televisi.material.map = new THREE.VideoTexture(video); 

kiri.add(lukisan.clone())
depan.add(lukisan.clone())
belakang.add(televisi)

// Tambah keterangan di mesin 
...
// Ukuran papan dalam unit Three.js
const boardWidth3D = 4;
const boardHeight3D = 2;

function createBoard(side) {
    // Buat elemen HTML untuk CSS3DObject
    const boardElement = document.createElement('div');
    boardElement.style.width = '400px';    
    boardElement.style.height = '200px';
    boardElement.style.background = 'white';
    ...
    boardElement.innerHTML = `<strong>BOARD ${side.toUpperCase()}</strong>`;

    // Buat CSS3DObject
    const board = new CSS3DObject(boardElement);

    // Tentukan posisi dan rotasi berdasarkan sisi
    if (side === 'kiri') {
        board.position.set(
            kiri.position.x + wallThickness/2 + 0.01, // sedikit di depan dinding kiri
            wallHeight / 2,                           // tengah tinggi dinding
            0                                         // tengah sumbu z
        );
        board.rotation.y = Math.PI / 2; // menghadap ke tengah ruangan
    } else if (side === 'kanan') {
        board.position.set(
            kanan.position.x - wallThickness/2 - 0.01, // sedikit di depan dinding kanan
            wallHeight / 2,
            0
        );
        board.rotation.y = -Math.PI / 2; 
    } else {
        console.warn('Side harus "kiri" atau "kanan"!');
        return null;
    }

    // Skala
    board.scale.set(boardWidth3D / 100, boardHeight3D / 100, 1);
...
// Contoh pemakaian:
// const boardKiri = createBoard('kiri');
const boardKanan = createBoard('kanan');
