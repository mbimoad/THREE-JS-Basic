...
loader.load(modelrack.href, item => {
    ...
    let tinggi = 0; 
    item.scene.traverse(function(e) {
        if(e.name.includes("Shelf_Supports")) wadah.push(e); 
        if(e.name == "Sides_1") {
            e.scale.y = 2; 
            e.updateWorldMatrix(true); 
            tinggi = getObjectSize(e).y 
        }
    })
    const wc = wadah[0].clone(); 
    wadah.push(wc)
    item.scene.add(wc); 

    const pan = wadah.length - 1; 
    const gap = tinggi / pan; 
    for(let i=0; i<=pan; i++) wadah[i].position.y = gap * i;
    createbox(0.009, true, wadah[0], 0.5, 0, 0);
    createbox(0.5, false, wadah[1], 0.5, 0, 0);
})

const createbox = (w,m,p,x,y,z) => {
    y = y+w/2; 
    if(!m) {
...
const getObjectSize = parent => {
    let boxs = new THREE.Box3().setFromObject(parent); 
