...
// Fungsi gposi sekarang fleksibel: ukuran object (objW,objH,objD), ukuran plane (planeW, planeH), boxHeight untuk tumpukan
function gposi(objW, objH, objD, planeW, planeH, boxHeight, s, i, j, k, n, cs=0) {
    const y = boxHeight / 2 + k * boxHeight;
    return {
         x: (-planeW / 2 + i * objW + objW / 2) + s.position.x,
         z: (-planeH / 2 + j * objD + objD / 2) + s.position.z,
         y: y,
        y2: y  + s.position.y,
        y3: cs + s.position.y,
        newname: `${n}-${i}-${j}`
...
function createHover(s, w, h, wg, hg, n) {
    ...
    for (let i = 0; i < wg; i++) {
        for (let j = 0; j < hg; j++) {
            ...
            const pos = gposi(cw, cs, ch, w, h, bh, s, i, j, 0, n, cs);
    ...
    for (let i = 0; i < wg; i++) {
        for (let j = 0; j < hg; j++) {
            for (let k = 0; k < stacked; k++) {
                ...
                const pos = gposi(bw, bh, bd, w, h, bh, p, i, j, k, n);
