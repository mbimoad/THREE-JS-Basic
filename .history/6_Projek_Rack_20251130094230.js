...
const getObjectSize = obj => {
    ...
    return {
        ...
        x: size.x, 
        y: size.y,
        z: size.z  // <--- TAMBAHKAN INI
    };
};

function planeShadowed(pos, col, row, parentMesh) {
    console.log(pos)
    const mesh = new THREE.Mesh(
        new THREE.PlaneGeometry(pos.z, 1, col, row),
    ...
    // convert world → local
    const worldPos = pos.cntr.clone();
    const localPos = parentMesh.worldToLocal(worldPos);

    mesh.position.copy(localPos);
    parentMesh.add(mesh); // <--- bukan scenes.add
    return mesh;
}
