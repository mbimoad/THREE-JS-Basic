...
function gposi(w, h, cellwidths, cellheight, boxHeight, ground, i, j, k, key) {
    return {
        x: (-w / 2 + i * cellwidths + cellwidths / 2) + ground.position.x,
        z: (-h / 2 + j * cellheight + cellheight / 2) + ground.position.z,
        y: (boxHeight / 2) + (k * boxHeight),
        newname: `${key}-${i}-${j}`
    };
}
function createHoverToPlaneGeometry(ground, w, h, Gwsegmn, Ghsegmn, key) {
    const cellwidths = w / Gwsegmn;
    const cellheight = h / Ghsegmn;
    const cellHovers = 0.01;
    const boxHeight = 0.3;
    for (let i = 0; i < Gwsegmn; i++) {
        for (let j = 0; j < Ghsegmn; j++) {
            const hover = boxes(cellwidths, cellHovers, cellheight, 0x00FF00, true, 0.0); 
            const pos   = gposi(w, h, cellwidths, cellheight, boxHeight, ground, i, j, 0, key);
...
function loopingBoxToPlaneGeometry(stacked, ground, w, h, Gwsegmn, Ghsegmn, key) {
    const cellwidths = w / Gwsegmn;
    const cellheight = h / Ghsegmn;
    ...
    for (let i = 0; i < Gwsegmn; i++) {
        for (let j = 0; j < Ghsegmn; j++) {
            for (let k = 0; k < stacked; k++) {
                const b   = boxes(boxWidth, boxHeight, boxDepth, Math.random() * 0xFFFFFF, false, 1); 
                const pos = gposi(w, h, cellwidths, cellheight, boxHeight, ground, i, j, k, key);
...
function createGround(w, h, Gwsegmn, Ghsegmn) {
    const g = new THREE.Mesh(
        new THREE.PlaneGeometry(w, h, Gwsegmn, Ghsegmn),
