...
// Misal datasbox sudah diinisialisasi seperti ini
let datasbox = [
    {
        rack: "Shelf_Supports_12_3-1-1",
        position: new THREE.Vector3(0.2651501893997188, 0.18979999930858557, 0)
...
// Fungsi mencari objek rack berdasarkan nama
...
// Initialize hover/box berdasarkan datasbox
datasbox.forEach(data => {
    const obj = findRackByName(data.rack);
    if (!obj) {
        console.warn("Objek rack tidak ditemukan:", data.rack);
        return;
    }

    // Contoh: membuat hover box kecil di posisi data.position
    const hov = boxes(0.1, 0.02, 0.1, 0x00ff00, true, 0.5);

    // position relatif ke parent rack
    const parentRack = obj.parent; // atau obj sendiri, tergantung ingin mengikuti transform
    const localPos = data.position.clone();
    parentRack.worldToLocal(localPos); // ubah dari world → local

    hov.position.copy(localPos);

    // tambahkan ke parent rack
    parentRack.add(hov);

    // simpan metadata jika perlu
    hov.userData.ww = obj;
    hoverbox.push(hov);
});
