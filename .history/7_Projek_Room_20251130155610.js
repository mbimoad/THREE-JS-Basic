...
modeload.load('./andy_room.gltf', item => {
    ...
    item.scene.traverse(i => {
        if(i.name === "Desk_Poly") {
            ...
            // Ambil posisi global Desk_Poly
            const worldPos = new THREE.Vector3();
            i.getWorldPosition(worldPos);
    
            // Tambahkan ke scene root, bukan ke Desk_Poly
            i.add(box);
    
            // Set posisi box sesuai posisi global Desk_Poly
            box.position.copy(worldPos);
    
            console.log("Box world pos:", box.position);
        }
    });
